...
def connect_db():
	sql = sqlite3.connect('C:/Users/NIKHIL/Desktop/question_answer_app/questions.db')
...
def question(question_id):
	user = get_current_user()
	db = get_db()
	question_cur = db.execute('select questions.question_text, questions.answer_text, askers.name as asker_name, experts.name as expert_name from questions join user as askers on askers.id = questions.asked_by_id join user as experts on experts.id = questions.expert_id where questions.id = ?', [question_id])
...
def unanswered():
	...
	if not user:
		return redirect(url_for('login'))
	db = get_db()
	questions_cur = db.execute('select questions.id, questions.question_text, user.name from questions join user on user.id = questions.asked_by_id where questions.answer_text is null and questions.expert_id = ?',[user['id']])
